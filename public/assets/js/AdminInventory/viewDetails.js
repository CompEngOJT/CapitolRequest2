...
function fetchWithdrawalRequests(productName, page = 1) {
    ...
    fetch('/admin/inventory/withdrawal-details', {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            document.getElementById('noRequestsMessage').classList.remove('hidden');
            document.getElementById('noRequestsMessage').textContent = 'Error loading requests: ' + data.message;
        }
    })
    .catch(error => {
        ...
        document.getElementById('loadingRequests').style.display = 'none';
        document.getElementById('noRequestsMessage').classList.remove('hidden');
        document.getElementById('noRequestsMessage').textContent = 'Error loading requests. Please try again.';
    });
}
